export default class App {
  ...
  start() {
    ...
    if (!this.initialized) {
      // reference all of the confetti elements in DOM
      this.confettis = this.template.querySelectorAll(".confetti");

      this.browserWidth = window.innerWidth;
      this.browserHeight = window.innerHeight;

      for (var i = 0; i < this.confettis.length; i++) {
        // set our confetti's initial position and related properties
        let initialXPos = this.getPosition(50, this.browserWidth);
        let initialYPos = this.getPosition(-50, -1 * this.browserHeight);
        let speed = 20 + Math.random() * 100;
        let size = 100;
        let shapeStyle = "shape";

        let classValue = this.classValues[
          Math.floor(Math.random() * this.classValues.length)
        ];

        let confettiElement = this.confettis[i];
        confettiElement.classList.add(classValue);

        // pick a random color from our predefined list of colors
        let color = this.colors[Math.floor(Math.random() * this.colors.length)];

        switch (classValue) {
          case "circle1":
            this.setConfettiStyle(this.circle1, color, confettiElement);
            shapeStyle = "outline";
            break;
          case "circle2":
            this.setConfettiStyle(this.circle2, color, confettiElement);
            shapeStyle = "solid";
            break;
          case "circle3":
            this.setConfettiStyle(this.circle3, color, confettiElement);
            shapeStyle = "diagonal";
            break;
          case "square1":
            this.setConfettiStyle(this.square1, color, confettiElement);
            shapeStyle = "outline";
            break;
          case "square2":
            this.setConfettiStyle(this.square2, color, confettiElement);
            shapeStyle = "solid";
            break;
          case "square3":
            this.setConfettiStyle(this.square3, color, confettiElement);
            shapeStyle = "diagonal";
            break;
          case "star1":
            this.setConfettiStyle(this.star1, color, confettiElement);
            shapeStyle = "outline";
            break;
          case "star2":
            this.setConfettiStyle(this.star2, color, confettiElement);
            shapeStyle = "solid";
            break;
          case "star3":
            this.setConfettiStyle(this.star3, color, confettiElement);
            shapeStyle = "diagonal";
            break;
          case "triangle1":
            this.setConfettiStyle(this.triangle1, color, confettiElement);
            shapeStyle = "outline";
            break;
          case "triangle2":
            this.setConfettiStyle(this.triangle2, color, confettiElement);
            shapeStyle = "solid";
            break;
          case "triangle3":
            this.setConfettiStyle(this.triangle3, color, confettiElement);
            shapeStyle = "diagonal";
            break;
          case "polygon1":
            this.setConfettiStyle(this.polygon1, color, confettiElement);
            shapeStyle = "outline";
            break;
          case "polygon2":
            this.setConfettiStyle(this.polygon2, color, confettiElement);
            shapeStyle = "solid";
            break;
          case "polygon3":
            this.setConfettiStyle(this.polygon3, color, confettiElement);
            shapeStyle = "diagonal";
            break;
          default:
            //no shape selected
        ...
        let confettiObject = new Confetti(
          confettiElement,
          speed,
          initialXPos,
          initialYPos,
        ...
        this.confettiObjects.push(confettiObject);
      }

      this.setupHeartPieces();
  ...
  setupHeartPieces() {
    var heartLeft = this.template.querySelector(".heartLeft");
    var heartRight = this.template.querySelector(".heartRight");
    var heartTriangle = this.template.querySelector(".heartTriangle");
    var heart = this.template.querySelector(".heart");

    heart.style.backgroundImage =
      "url('data:image/svg+xml;base64," + window.btoa(this.heart()) + "')";
    heartLeft.style.backgroundImage =
      "url('data:image/svg+xml;base64," +
      window.btoa(this.part_heartLeft()) +
      "')";
    heartRight.style.backgroundImage =
      "url('data:image/svg+xml;base64," +
      window.btoa(this.part_heartRight()) +
      "')";
    heartTriangle.style.backgroundImage =
      "url('data:image/svg+xml;base64," +
      window.btoa(this.part_heartTriangle()) +
      "')";
  }

  animate() {
    for (var i = 0; i < this.confettiObjects.length; i++) {
      var confettiObject = this.confettiObjects[i];
      if (confettiObject.exists) {
        confettiObject.update();
      }
    }
  ...
  setConfettiStyle(style, color, el) {
    el.style.backgroundImage =
      "url('data:image/svg+xml;base64," + window.btoa(style(color)) + "')";
  }
}
...
class Confetti {
  constructor(
    ...
    // set initial confetti properties
    ...
    if (shapeStyle == "solid") {
      this.scale = 220 / 800;
    } else if (shapeStyle == "outline") {
      var bigOutline = this.getRandomNumber(0, 10);
      if (bigOutline > 5) {
        this.scale = 120 / 800;
      } else {
        this.scale = 160 / 800;
      }
    } else if (shapeStyle == "diagonal") {
      var bigDiagonal = this.getRandomNumber(0, 10);
      if (bigDiagonal > 9) {
        var leftRight = this.getRandomNumber(0, 10);

        if (leftRight > 5) {
          this.xPos = this.getRandomNumber(100, 400);
        } else {
          this.xPos = this.getRandomNumber(
            browserWidth - 400,
            browserWidth - 100
          );
        }
    ...
    this.rotation = this.getRandomNumber(0, 360);
    this.r1 = this.getRandomNumber(20, 50);
    this.r2 = this.getRandomNumber(1, 3);

    this.browserHeight = browserHeight;
    this.exists = true;

    // declare variables used for confetti's motion
    this.counter = 0;
    this.sign = Math.random() < 0.5 ? 1 : -1;
  }

  update() {
    if (this.yPos < this.browserHeight + 100) {
      // using some trigonometry to determine our x and y position
      this.counter += this.speed / 1000;

      this.xPos += this.sign * this.r2 * 1.5 * Math.cos(this.counter);
      this.yPos += 7 + 2 * Math.sin(this.counter);

      //this.scale = .1 + (this.speed / 700) + Math.abs(Math.cos(2 * this.counter) / 100);
      this.rotation += this.speed / 30;

      // setting our confetti's position and size
      this.setTransform(
  ...
  //
  // A performant way to set your confetti's position
  // - skew(${rotation / 100}deg,
  //
  setTransform(xPos, yPos, scale, rotation, el) {
    el.style.transform = `translate3d(${xPos}px, ${yPos}px, 0) scale(${scale}, ${scale}) rotateY(${rotation *
      3}deg) rotateX(${rotation * 2}deg) rotateZ(${rotation}deg)`;
  }
}
